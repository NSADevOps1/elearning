...
resource "aws_lb_listener" "e-learning-alb-listener" {
...
resource "aws_ecs_service" "e-learning-service" {
  ...
  depends_on = [aws_lb_listener.e-learning-alb-listener, aws_iam_role_policy_attachment.ecs-task-execution-role_policy]
}
...
resource "aws_route53_record" "calebcreative_record" {
  ...
  name    = "stage.calebcreatives.click"
  type    = "A"
